...
def main():
    ...
    if numberOfProducers > 0:
        ...
        if abs(totalCurrentDemand) > totalCurrentSupply:
            ...
            """ hinzufügen von producer nodes"""
            for producerId, supplyValue in integerSupply.items():
                # add producer node
                G.add_node(producerId, demand=supplyValue)

            """ hinzufügen von consumer nodes"""
            for consumerId, demandValue in proportionalDemand.items():
                # add consumer node
                G.add_node(consumerId, demand=demandValue)

            """ neuer Fall nämlich wenn kein Dummy benötigt wird muss auch hinzugefügt werden"""

            # mit dieser if-bedingung wird analyisiert, ob überhaupt ein dummy eingefügt werden musste
            if len(dummy) > 0:

                """ hinzufügen von dummy Knoten und Kanten"""
                if dummy['type'] == 'propConsumerDummy':
                    G.add_node('propConsumerDummy', demand=-dummy['value'])
                    # füge zusätzliche Kanten für den dummyNode hinzu
                    for producerId in integerSupply.keys():
                ...
                if dummy['type'] == 'supplyDummy':
                    ...
                    # hinzufügen der zusätzlichen Kanten
                    for consumerId in integerDemand.keys():
        ...
        for matchedhouseholds in tradingpairs:
            ...
            #  print(matchedhouseholds)

            """ key error handling has to be done since dummy household to balance the
                problem isn't in cost json"""
            try:
